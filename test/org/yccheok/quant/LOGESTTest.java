...
public class LOGESTTest {
    ...
    /**
     * Test of growth_rate method, of class LOGEST.
     */
    @Test
    public void testGrowth_rate() {
        // http://faq.betterinvesting.org/index.php?action=artikel&cat=3&id=16&artlang=en
        double[] y = {6207, 8279, 13931, 19315, 24006, 32479, 42905, 65225, 108249, 156508};
        double[] x = {2003, 2004, 2005, 2006, 2007, 2008, 2009, 2010, 2011, 2012};

        double expResult = 0.4165506277448576;
        double result = LOGEST.growth_rate(y, x);
        assertEquals(expResult, result, 0.0);
        ...
        y = new double[]{1.0, -0.5, 0, -0.3, 6.0};
        x = new double[]{2013, 2014, 2015, 2016, 2017};
        
        expResult = Math.pow(6.0/1.0, 1.0/4.0) - 1;
        result = LOGEST.growth_rate(y, x);
        assertEquals(expResult, result, 0.00000001);       
        
        y = new double[]{6.0, -0.5, 0, -0.3, 1.0};
        x = new double[]{2013, 2014, 2015, 2016, 2017};
        
        expResult = Math.pow(1.0/6.0, 1.0/4.0) - 1;
        result = LOGEST.growth_rate(y, x);
